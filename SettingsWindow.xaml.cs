...
public partial class SettingsWindow : Window
{
    ...
    {
        try{var kind=(AiProvider.SelectedItem as ComboBoxItem)?.Content?.ToString()??"local";if(kind=="local"){System.Windows.MessageBox.Show("localモードは外部接続なしだよ。","るか");return;}if(kind=="ruka"){if(!Uri.TryCreate(RukaEndpoint.Text.Trim(),UriKind.Absolute,out var rukaEp))throw new InvalidOperationException("Ruka AIエンドポイントが正しくありません。");using var rukaClient=new HttpClient{Timeout=TimeSpan.FromSeconds(20)};IAiProvider rukaTest=new RukaCloudProvider(rukaClient,rukaEp.ToString(),"接続テスト用。短く返答してください。",_providerSettings.InstallationId);var rukaReply=await rukaTest.GenerateAsync("接続テスト。『接続OK』と短く返答してください。",[]);System.Windows.MessageBox.Show("Ruka AIサーバーから返答を受信したよ。\\n\\n"+rukaReply,"Ruka AI接続成功");return;}if(!Uri.TryCreate(AiEndpoint.Text.Trim(),UriKind.Absolute,out var ep))throw new InvalidOperationException("エンドポイントが正しくありません。");var key=string.IsNullOrWhiteSpace(AiApiKey.Password)?Environment.GetEnvironmentVariable("RUKA_AI_API_KEY")??"":AiApiKey.Password.Trim();using var client=new HttpClient{Timeout=TimeSpan.FromSeconds(20)};IAiProvider providerTest=kind=="openai"?new OpenAiCompatibleProvider(client,ep.ToString(),key, AiModel.Text.Trim(),"接続テスト用。短く返答してください。"):new HttpAiProvider(client,ep.ToString(),10);var providerReply=await providerTest.GenerateAsync("接続テスト。『接続OK』と短く返答してください。",[]);System.Windows.MessageBox.Show("実際のサーバーから返答を受信したよ。\\n\\n"+providerReply,"AI接続成功");}catch(Exception ex){System.Windows.MessageBox.Show("実接続テスト失敗。\\n"+ex.Message,"るか",MessageBoxButton.OK,MessageBoxImage.Warning);}
    }
