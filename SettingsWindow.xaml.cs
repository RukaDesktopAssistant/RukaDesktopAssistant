...
public partial class SettingsWindow : Window
{
    ...
    {
        try{var kind=(AiProvider.SelectedItem as ComboBoxItem)?.Content?.ToString()??"local";if(kind=="local"){MessageBox.Show("localモードは外部接続なしだよ。","るか");return;}if(!Uri.TryCreate(AiEndpoint.Text.Trim(),UriKind.Absolute,out var ep))throw new InvalidOperationException("エンドポイントが正しくありません。");var key=string.IsNullOrWhiteSpace(AiApiKey.Password)?Environment.GetEnvironmentVariable("RUKA_AI_API_KEY")??"":AiApiKey.Password.Trim();using var client=new HttpClient{Timeout=TimeSpan.FromSeconds(20)};IAiProvider test=kind=="openai"?new OpenAiCompatibleProvider(client,ep.ToString(),key, AiModel.Text.Trim(),"接続テスト用。短く返答してください。"):new HttpAiProvider(client,ep.ToString(),10);var reply=await test.GenerateAsync("接続テスト。『接続OK』と短く返答してください。",[]);MessageBox.Show("実際のサーバーから返答を受信したよ。\\n\\n"+reply,"AI接続成功");}catch(Exception ex){MessageBox.Show("実接続テスト失敗。\\n"+ex.Message,"るか",MessageBoxButton.OK,MessageBoxImage.Warning);}
    }
    ...
    private void ClearMemory_Click(object sender,RoutedEventArgs e){if(MessageBox.Show("長期記憶をすべて削除する？","確認",MessageBoxButton.YesNo)==MessageBoxResult.Yes){_memory.Clear();RefreshMemory();}}
    private void ClearConversation_Click(object sender,RoutedEventArgs e){if(MessageBox.Show("会話履歴をすべて削除する？","確認",MessageBoxButton.YesNo)==MessageBoxResult.Yes)_conversation.Clear();}
    ...
    private void SaveGame_Click(object sender,RoutedEventArgs e){if(GameSelector.SelectedItem is not string name)return;_games.Set(name,new GameProfile(GameShow.IsChecked==true,GameScale.Value,"side",GameVoice.IsChecked==true,GameMoveSide.IsChecked==true));MessageBox.Show("ゲーム設定を保存したよ。","るか");}
    private void LoadPersonalities(){PersonalitySelector.ItemsSource=_personalities.Profiles.Select(x=>x.Name).ToList();if(_personalities.Profiles.Count>0)PersonalitySelector.SelectedIndex=0;}
